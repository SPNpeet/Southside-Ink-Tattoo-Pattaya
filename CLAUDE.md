...
- **ห้ามใส่ป้ายประเภทใต้รูปผลงาน** (ลูกค้าสั่ง) แต่ปุ่มสไตล์ต้องกรองรูปได้จริง (พี่ peet สั่ง 6 ก.ย. 2569)
  หมวดของแต่ละรูปอยู่ใน `WORK_STYLES` เรียงตาม `WORKS` จัดจากการดูรูปจริง เพิ่มรูปใหม่ต้องเพิ่มหมวดคู่กันเสมอ
  ชิปใน About และแท็กช่างกดแล้วกระโดดไปกรองผ่าน `STYLE_ALIAS`
- **โหมดไทย/อังกฤษต้องแยกภาษาสมบูรณ์** ข้อความทุกอันอยู่ใน `I18N` ห้ามเขียนภาษาไทยตรง ๆ ใน JSX
  ยกเว้นชื่อแบรนด์ ชื่อสไตล์ (ศัพท์สากล) และช่องทางติดต่อ
...
- ลิงก์ LINE ต้องเป็น `https://line.me/ti/p/VzL1rgJr-E` (ถอดจาก QR ของร้าน) ห้ามใช้ `line.me/ti/p/Ponair1`
  เพราะเปิดแล้วเป็นหน้า Error ทั้งที่ตอบ HTTP 200 ตรวจลิงก์ LINE ต้องดูเนื้อหาหน้า
- ปุ่ม/ชิปทุกอันต้องกดแล้วเกิดผล ห้ามมี element ที่ดูเหมือนปุ่มแต่กดไม่ได้

- ห้ามแต่งข้อมูลที่พิสูจน์ไม่ได้ (ตัวเลข ผลงาน รีวิว) เว็บนี้ลูกค้าจริงใช้ตัดสินใจ
  รีวิวใน `REVIEWS` ต้องคัดจากหน้า Google ของร้านเท่านั้น (https://maps.app.goo.gl/DjVrFQs2Y1T8Ni7N7)
  ใช้ชื่อผู้รีวิวตามที่ Google แสดง คำแปลต้องมีป้าย "แปลจาก..." เสมอ
  ตัวเลขคะแนน/จำนวนรีวิว (ตอนนี้ 5.0 จาก 47) มี 4 ที่ต้องแก้พร้อมกัน: `heroKicker` สองภาษา, `reviewsLink` สองภาษา,
  `index.html` (description + JSON-LD reviewCount) และรูป `public/og.jpg`
...
## 2.1 ทิศทางดีไซน์ (พี่ peet สั่ง 7 ก.ย. 2569)

- ห้ามกล่อง CTA ไล่เฉดสีสำเร็จรูป ใช้ของจริงของร้านแทน (รูปป้ายหน้าร้าน สถานะเปิด/ปิดสด ช่องทางจริง)
